...
# changelog:
# 2018.12.11 Blender 2.80 Porting
...
def equalize(operator, context, use_pack, rotate, margin, use_active, ):
    ...
    obs = [ob for ob in context.selected_objects if ob.name != ao.name and ob.type == 'MESH']
...
class UVEqualizePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        self.layout.operator("uv.uv_equalize")
...
def register():
    bpy.utils.register_class(UVEqualize)
    bpy.utils.register_class(UVEqualizePanel)
    bpy.types.VIEW3D_MT_object.append(menu_func)


def unregister():
    bpy.utils.unregister_class(UVEqualize)
    bpy.utils.unregister_class(UVEqualizePanel)
